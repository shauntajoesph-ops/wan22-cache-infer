...
from wan.inference import INFERENCE_ONLY
...
def generate(args):
    ...
    # INFERENCE: Enforce inference-only mode unless explicitly disabled.
    if not INFERENCE_ONLY:
        logging.warning(
            "WAN_INFERENCE_ONLY=0 set; training APIs are not blocked. Proceed with caution.")

    # OPTIM: Default CPU offload toggled based on world size.
    # - Single-GPU: enable offload to lower peak VRAM.
    # - Multi-GPU: disable offload to avoid PCIe thrash and let FSDP/SP handle memory.
    ...
    # OPTIM: Sequence Parallel (Ulysses) reduces attention KV memory by splitting sequence across ranks.
    if args.ulysses_size > 1:
        assert args.ulysses_size == world_size, f"The number of ulysses_size should be equal to the world size."
        init_distributed_group()

    # OPTIM: Prompt extension is done on rank 0 then broadcast; avoids redundant LL(M/VLM) calls and host<->device copies.
    ...
    # OPTIM: Construct the specific pipeline with flags that affect memory/speed:
    # - t5_fsdp / dit_fsdp: shard models across GPUs to reduce memory.
