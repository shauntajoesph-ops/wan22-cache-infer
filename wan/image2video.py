...
# Legacy per-module cache states removed; use CacheManager only.
...
class WanI2V:
    ...
    def __init__(
        ...
        # OPTIM: Disable init-on-CPU when using FSDP/SP to avoid repeated device transfers.
        if t5_fsdp or dit_fsdp or use_sp:
            self.init_on_cpu = False

        shard_fn = partial(shard_model, device_id=device_id)
        # OPTIM: T5 can be FSDP-sharded or kept on CPU to save VRAM.
        ...
        # OPTIM: VAE encode/decode under AMP for throughput.
        self.vae = Wan2_1_VAE(
        ...
        # OPTIM: Two-expert MoE across timesteps; swap experts by boundary.
        ...
        # OPTIM: Sequence Parallel reduces KV memory footprint by sharding sequence.
        if use_sp:
    ...
    def _attach_teacache_state(self, timesteps_len: int):
        # Legacy no-op: unified CacheManager handles gating/telemetry now.
        return
    ...
    def _log_teacache_stats(self):
        # Legacy no-op: use CacheManager.summary() logging in generate()
        return
    ...
    def _configure_model(self, model, use_sp, dit_fsdp, shard_fn,
        ...
        """
        # OPTIM: eval()+no-grad to avoid autograd graphs and reduce sync.
        model.eval().requires_grad_(False)

        if use_sp:
            # OPTIM: Replace attention/forward with SP variants to split sequence across GPUs.
        ...
        if dit_fsdp:
            ...
            if convert_model_dtype:
                # OPTIM: Convert to param_dtype (bf16/fp16) to speed GEMMs.
