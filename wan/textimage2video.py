...
class WanTI2V:
    ...
    def __init__(
        ...
        # OPTIM: When using FSDP/SP, init-on-CPU is disabled to avoid repeated device transfers.
        if t5_fsdp or dit_fsdp or use_sp:
            self.init_on_cpu = False

        shard_fn = partial(shard_model, device_id=device_id)
        # OPTIM: T5 can be FSDP-sharded or kept on CPU for memory headroom.
        ...
        # OPTIM: VAE runs under AMP autocast to speed encode/decode.
        self.vae = Wan2_2_VAE(
        ...
        # OPTIM: Single expert backbone (TI2V-5B); will run with SP/FSDP and bf16 params if configured.
    ...
    def _configure_model(self, model, use_sp, dit_fsdp, shard_fn,
        ...
        """
        # OPTIM: Inference flags ensure no autograd, lower sync overhead.
        model.eval().requires_grad_(False)

        if use_sp:
            # OPTIM: Sequence-parallel attention and forward to shard sequence across ranks.
        ...
        if dit_fsdp:
            # OPTIM: Shard parameters and use mixed precision buffers.
            model = shard_fn(model)
        else:
            if convert_model_dtype:
                # OPTIM: Convert parameters to param_dtype (bf16/fp16) for faster matmuls.
    ...
    def t2v(self,
        ...
        # OPTIM: Just-in-time move of T5 to GPU; offload when done.
        if not self.t5_cpu:
        ...
        # evaluation mode
        # OPTIM: AMP autocast + no_sync for FSDP to reduce overhead.
        with (
                torch.amp.autocast('cuda', dtype=self.param_dtype),
                inference_guard(),
                no_sync(),
        ):
            # Ensure TeaCache state is freshly attached/reset for this run
            # (do it here as an extra guard in case callers invoke generate() twice)
            # Note: _attach_teacache_state also handles resetting existing states.
            # We call it once actual timesteps are known below.

            # OPTIM: Device-resident solver timesteps avoid host transfers.
